...
parser.add_argument('--verify_random_forest', dest='verify_random_forest', 
                                              type=lambda x:bool(distutils.util.strtobool(x)), default=False,
                                              help='Add custom data to training data')
parser.add_argument('--verify_random_forest_file', type=str, default='verify_data_bert.csv', help='Folder in which to save model and logs')
...
parser.add_argument('--random_train_val', dest='random_train_val', 
                                              ...
                                              help='Add custom data to training data')                                                                                  
parser.add_argument('--r2_threshold_extra', type=float, default=0, help='Folder in which to save model and logs')
parser.add_argument('--min_iters_extra', type=int, default=200, help='Folder in which to save model and logs')
parser.add_argument('--split_interval', type=tuple, default=(0,5,7,9), help='Folder in which to save model and logs')

parser.add_argument('--r2_threshold_inter', type=float, default=0, help='Folder in which to save model and logs')
...
file_test.drop(columns=['Index'],inplace=True)
...
model = '_bert' if args.sample_data_file == 'data_test.csv' else '_distil_roberta'
...
file_test['Dataset'] = file_test['Dataset'].map(convert_dataset)
datasets = [
...
for t in itertools.count():
    ...
    x_train, x_val, y_train, y_val = train_test_split(file.drop(columns='ASR'), np.array(file['ASR']), test_size = 0.3, random_state = 0)
    data_test = file_test
...
#     if (max(r2_rf) > args.r2_threshold_extra and t > args.min_iters_extra) or split_specified:
#         print('Feature Importance')
#         print('*'*10)
...
#     if (max(r2_rf) > args.r2_threshold_inter and t > args.min_iters_inter):
#         print('Feature Importance')
#         print('*'*10)
